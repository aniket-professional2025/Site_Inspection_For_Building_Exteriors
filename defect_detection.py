...
# The function to get the detections
def get_defects(image_path, output_path = None, detection_threshold = 0.35):
    ...
    # Run each defect label individually
    defect_labels = ["crack", "dampness on the wall", "fungal growth", "flaking", "uneven surface", "holes"]
    draw = ImageDraw.Draw(image)
    detected_labels = []

    for label in defect_labels:
        print(f"[INFO] Testing label: {label}")
        ...
        for box, score in zip(results["boxes"], results["scores"]):
            ...
            draw.text((box[0], box[1] - 10), f"{label} ({score_val})", fill="red")
...
if __name__ == "__main__":
    image_path = r"C:\Users\Webbies\Jupyter_Notebooks\Berger_Site_Inspection_Exterior\Images\OrgImages\TestMain_1.png"
    labels = get_defects(image_path = image_path, detection_threshold = 0.26)
    print(labels)
